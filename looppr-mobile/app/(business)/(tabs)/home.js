  ...
  return (
    <SafeAreaView style={{ flex: 1 }} className="bg-bg" edges={['top']}>
      <ScreenHeader
        ...
        subtitle={`Hazel St Airbnb · ${properties?.length ?? 0} properties · net-30`}
      ...
      <ScrollView className="px-lg" contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        <Animated.View entering={rise()}>
          ...
          >
            ...
            <Text className="font-body text-[12px] text-[#F1EFFE] mb-lg">
              Auto-books from your Airbnb calendar · commercial rate $1.40/lb
            </Text>
            <Button
              title={planRequested ? 'Pickup requested ✓' : 'Request extra pickup'}
              variant="secondary"
        ...
        <View className="gap-sm">
          {(properties ?? []).map((property) => (
            <Animated.View key={property.id} entering={rise()}>
